...
```json
{
  ...
  "required": ["core_category_candidates", "recommendation"],
  "properties": {
    "core_category_candidates": {
      "type": "array",
      "description": "Categories evaluated as potential core categories, ordered by decreasing suitability (most central first). All candidates must EXIST in categories_summary.",
      "items": {
        "type": "object",
        "additionalProperties": false,
        "required": ["category_label", "centrality_score", "rationale", "connected_categories", "key_hypothesis_refs"],
        "properties": {
          "category_label": {
            "type": "string",
            "description": "EXACT category name as it appears in categories_summary. Must match letter-for-letter including case and special characters."
          },
          "centrality_score": {
            "type": "number",
            "minimum": 0.0,
            "maximum": 1.0,
            "description": "Qualitative centrality score (0.0 = peripheral, 1.0 = maximally central). NOT algorithmic — represents your qualitative judgment of how central this category is to the entire system."
          },
          "rationale": {
            "type": "string",
            "description": "Qualitative reasoning covering the 4 criteria: centrality (connections to other categories), explanatory power (does it explain WHY?), frequency with variation, and theoretical grab. Cite specific hypothesis IDs [H1], [H2], etc. as evidence."
          },
          "connected_categories": {
            "type": "array",
            "items": {"type": "string"},
            "description": "List of other category labels this category connects to (via hypotheses). Must be existing category labels from categories_summary."
          },
          "key_hypothesis_refs": {
            "type": "array",
            "items": {"type": "string"},
            "description": "References to specific hypotheses that support this candidate as core category. Use [H1], [H2], etc. format matching the hypotheses_summary."
          }
        }
      }
    },
    "recommendation": {
      "type": "string",
      "description": "Ranked recommendation narrative. Explain which category you recommend as core category and why. If multiple strong candidates exist, explain the trade-offs. The researcher (HITL) will make the final choice. Write in the language indicated by language_code."
    },
    "no_suitable_core": {
      "type": "boolean",
      "description": "true if no existing category reaches core category level. More data or more hypothesis development may be needed."
    },
    "no_suitable_rationale": {
      "type": "string",
      "description": "If no_suitable_core=true: explanation of what is missing (insufficient connections, low explanatory power, categories too descriptive, need more documents)."
    }
  }
}
```
