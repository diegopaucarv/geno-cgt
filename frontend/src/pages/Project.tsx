...
export default function ProjectDetail() {
  ...
  function resetStages(presets?: Record<string, StageStatus>) {
    ...
    PIPELINE_STAGES.forEach((s) => (init[s.key] = presets?.[s.key] || "pending"));
  ...
  async function runPipeline(forceAll: boolean = false) {
    ...
    if (isContinue) {
      ...
      await fetch("/api/v1/admin/workers/nlp/start", { method: "POST", headers: { Authorization: auth } });
      await fetch("/api/v1/admin/workers/heavy/start", { method: "POST", headers: { Authorization: auth } });
    ...
    if (forceAll) {
      ...
      for (const dl of pipelineLog.documents) {
    ...
    if (todo.length === 0) {
      if (!isContinue) { updateStage("segment", "done"); }
    ...
    updateStage("categories", "done");
    updateStage("done", "done");
    setPipelineMsg(abortRef.current ? "⏹ Pipeline cancelado." : "✅ Pipeline completado.");

    if (abortRef.current) {
      await fetch("/api/v1/admin/workers/heavy/stop", { method: "POST", headers: { Authorization: auth } }).catch(() => {});
      await fetch("/api/v1/admin/workers/nlp/stop", { method: "POST", headers: { Authorization: auth } }).catch(() => {});
    }
  ...
  function getEstadoBadge(doc: Document): { text: string; color: string; bg: string } {
    const log = getDocLog(doc.id);
    if (!log) {
      // Fallback
      switch (doc.estado) {
        case "listo": return { text: "✓ Listo", color: "#3FB950", bg: "#3FB95022" };
        case "segmentado": return { text: "✂️ Segmentado", color: "#3FB950", bg: "#3FB95022" };
        case "error": return { text: "✕ Error", color: "#F85149", bg: "#F8514922" };
        default: return { text: "⬜ Crudo", color: "#8B949E", bg: "#8B949E22" };
      }
    }
    // Use real log data
    if (log.steps.agents_done && log.steps.coded) return { text: "✓ Listo", color: "#3FB950", bg: "#3FB95022" };
    if (log.steps.segmented && !log.steps.coded) return { text: "✂️ Segmentado", color: "#D29922", bg: "#D2992222" };
    if (log.steps.segmented) return { text: "✂️ Segmentado", color: "#3FB950", bg: "#3FB95022" };
    if (log.steps.text_extracted) return { text: "📄 Con texto", color: "#58A6FF", bg: "#58A6FF22" };
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
            ...
            <span style={{
              ...
              background: docsDone === docs.length && docs.length > 0 ? "#3FB95022" : "#8B949E22",
              color: docsDone === docs.length && docs.length > 0 ? "#3FB950" : "#8B949E",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{
              ...
              <button
                onClick={() => { setGlobalViewMode("original"); setViewModeOverride({}); }}
                style={{
                  padding: "3px 10px", border: "none", fontSize: 11, cursor: "pointer",
                  background: globalViewMode === "original" ? "#A371F7" : "transparent",
                  color: globalViewMode === "original" ? "#FFF" : "#8B949E",
                }}
              >📄 Orig</button>
              <button
                onClick={() => { setGlobalViewMode("segmented"); setViewModeOverride({}); }}
                style={{
                  padding: "3px 10px", border: "none", fontSize: 11, cursor: "pointer",
                  background: globalViewMode === "segmented" ? "#A371F7" : "transparent",
            ...
            <Link
              ...
              onClick={(e) => { if (!playgroundReady) e.preventDefault(); }}
              title={playgroundReady ? "Explorar el modelo teórico" : "Necesitás ejecutar el pipeline primero"}
              style={{
                padding: "3px 10px", borderRadius: 6, border: "1px solid #21262D",
                background: playgroundReady ? "#3FB95022" : "#D2992222",
                color: playgroundReady ? "#3FB950" : "#D29922",
                fontSize: 11, fontWeight: 600, cursor: playgroundReady ? "pointer" : "not-allowed",
                textDecoration: "none", opacity: playgroundReady ? 1 : 0.6,
              }}
            >
            ...
            <button
              ...
              style={{
                ...
                color: showLog ? "#A371F7" : "#8B949E", fontSize: 11, cursor: "pointer",
              }}
            >
        ...
        {showLog && docs.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column", gap: 3, paddingTop: 4, borderTop: "1px solid #21262D" }}>
            {docs.map((d) => {
              ...
              return (
                <div key={d.id} style={{
                  ...
                  padding: "3px 6px", borderRadius: 3, background: "#0D1117", fontSize: 11,
                }}>
                  <span style={{ color: "#E6EDF3", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, marginRight: 8 }}>
                    {d.original_filename}
                  </span>
                  <span style={{ display: "flex", gap: 4, alignItems: "center" }}>
                    {log?.segments_count ? (
                      <span style={{ fontSize: 9, color: "#8B949E" }}>{log.segments_count} seg</span>
                    ) : null}
                    {log?.codes_count ? (
                      <span style={{ fontSize: 9, color: "#8B949E" }}>{log.codes_count} cod</span>
                    ) : null}
                    <span style={{ fontSize: 9, padding: "1px 6px", borderRadius: 999, background: badge.bg, color: badge.color, whiteSpace: "nowrap" }}>
                      {badge.text}
                    </span>
                  </span>
                </div>
        ...
        <div style={{
          ...
          justifyContent: "flex-start", paddingTop: 4, borderTop: "1px solid #21262D",
        }}>
          {pipelineRunning ? (
            <button
              onClick={() => {
              ...
              style={{
                ...
                border: "1px solid #F8514944", background: "#F8514922",
                color: "#F85149", fontSize: 12, fontWeight: 600, cursor: "pointer",
              }}
            >
              ⏹ Cancelar pipeline
            </button>
            ...
            <>
              <button onClick={() => runPipeline(false)} disabled={docs.length === 0}
                style={{ padding: "10px 24px", borderRadius: 6, border: "none", cursor: "pointer", background: "linear-gradient(135deg, #3FB950, #58A6FF)", color: "#FFF", fontSize: 14, fontWeight: 600, boxShadow: "0 2px 12px rgba(63,185,80,0.3)" }}>
                ▶ Completar agentes ({docsNeedAgents} doc{docsNeedAgents !== 1 ? "s" : ""})
              </button>
              <button onClick={() => { if (confirm("¿Re-ejecutar TODO desde cero?")) runPipeline(true); }}
                style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #F8514944", background: "transparent", color: "#F85149", fontSize: 11, cursor: "pointer" }}>
                🔄 Forzar todo
              </button>
            </>
          ) : (
            <>
              {docs.length > 0 && (
                <button onClick={() => runPipeline(false)} disabled={docs.length === 0}
                  style={{ padding: "10px 24px", borderRadius: 6, border: "none", cursor: "pointer", background: "linear-gradient(135deg, #A371F7, #3FB950)", color: "#FFF", fontSize: 14, fontWeight: 600, boxShadow: "0 4px 24px rgba(163,113,247,0.3)" }}>
                  🧠 Ejecutar Pipeline IA
                </button>
              )}
              {docsDone > 0 && (
                <button onClick={() => { if (confirm("¿Re-ejecutar desde cero?")) runPipeline(true); }}
                  style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #F8514944", background: "transparent", color: "#F85149", fontSize: 11, cursor: "pointer" }}>
                  🔄 Re-ejecutar
                </button>
              )}
          ...
          {pipelineMsg && pipelineRunning && (
            <span style={{ fontSize: 11, color: "#A371F7" }}>{pipelineMsg}</span>
          )}
        </div>
      </div>
        ...
        >
          ...
          >
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
              <h2 style={{ margin: "0 0 8px", fontSize: 18, fontWeight: 700, color: "#E6EDF3", textAlign: "center", flex: 1 }}>
                {pipelineRunning
              ...
              {!pipelineRunning && (
                <button onClick={() => setShowPipelineOverlay(false)}
                  style={{ padding: "2px 8px", borderRadius: 4, border: "1px solid #30363D", background: "transparent", color: "#8B949E", fontSize: 16, cursor: "pointer", lineHeight: 1 }}>
                  ✕
                </button>
              )}
            </div>
